const app = Vue.createApp({
    ...
    methods:
    {
        ...
        /**********************************************************/
        estimateStatsMembers(votes, most, least) {
            let order_congressMembers = [...this.congressMembers].filter(member => member.total_votes > 0)
            order_congressMembers.sort((memberini, memberfin) => {
                if (memberini[votes] > memberfin[votes]) { return 1 }
                if (memberini[votes] < memberfin[votes]) { return -1 }
                return 0;
            })
            for (let i = 0; i < (Math.round(this.congressMembers.length * 0.1)); i++) { this.congressStats[most].push(order_congressMembers[i]) }

            for (let j = order_congressMembers.length - 1;
                j > order_congressMembers.length - 1 - (Math.round(this.congressMembers.length * 0.1)); j--) { this.congressStats[least].push(order_congressMembers[j]) }
        }
    ...
    computed:
    {
        ...
        selectPartyMembers() {
            let selMembers = []
            selMembers = this.congressMembers.filter(member =>
                this.partys.includes(member.party) &&
                (member.state === this.state || this.state === ""))
            return selMembers;
        },
        ...
        paintPartyTable() {
            let partyTable = [
                ...
                {
                    party: 'Independents',
                    num_reps: this.congressStats.reublicans.length,
